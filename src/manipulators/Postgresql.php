...
class Postgresql extends AbstractDatabaseManipulator
{
    ...
    protected function _dropAutoPrimaryKey($details) {
        ...
        $this->query(
                sprintf(
                        'ALTER TABLE %s ALTER COLUMN "%s" SET DEFAULT NULL', $this->buildTableName($details['table'], $details['schema']), $details['column'], $sequence
                )
        );
    ...
    protected function _addForeignKey($details) {
        $this->query(
                sprintf(
                        'ALTER TABLE %s ADD CONSTRAINT "%s" FOREIGN KEY ("%s") REFERENCES %s ("%s") MATCH FULL ON DELETE %s ON UPDATE %s', $this->buildTableName($details['table'], $details['schema']), $details['name'], implode('","', $details['columns']), $this->buildTableName($details['foreign_table'], $details['foreign_schema']), implode('","', $details['foreign_columns']), $details['on_delete'] == '' ? 'NO ACTION' : $details['on_delete'], $details['on_update'] == '' ? 'NO ACTION' : $details['on_update']
                )
        );
    }
    ...
    protected function _addIndex($details) {
        $this->query(
                sprintf(
                        'CREATE INDEX %s "%s" ON %s ("%s")', $details['unique'] ? 'UNIQUE' : '', $details['name'], $this->buildTableName($details['table'], $details['schema']), implode('", "', $details['columns'])
                )
        );
    }
